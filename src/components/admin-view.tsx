...
function EditForm({ senior, onClose }: { senior: Senior; onClose: () => void }) {
  ...
  return (
    <>
      <p className="text-base font-semibold text-yellow-800 mb-4">{senior.name} 님 정보 수정</p>
      <form action={formAction} className="grid grid-cols-2 gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input name="name" defaultValue={senior.name}
            className="h-10 rounded-xl border border-gray-300 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900" />
        ...
        <div className="flex flex-col gap-1">
          ...
          <select name="region" defaultValue={senior.region}
            className="h-10 rounded-xl border border-gray-300 px-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-gray-900">
        ...
        <div className="col-span-2 flex flex-col gap-1">
          ...
          <textarea name="memo" defaultValue={senior.memo ?? ''} rows={2}
            className="rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 resize-none" />
        </div>
        <div className="col-span-2 flex gap-3 justify-end">
          <button type="button" onClick={onClose}
            className="h-10 px-4 rounded-xl border border-gray-300 text-sm font-semibold text-gray-700 hover:bg-gray-100 transition-colors">
            취소
          </button>
          <button type="submit" disabled={isPending}
            className="h-10 px-4 rounded-xl bg-gray-900 text-white text-sm font-semibold hover:bg-gray-700 transition-colors disabled:opacity-50">
            {isPending ? '저장 중…' : '저장'}
          </button>
        </div>
      </form>
  ...
  return (
    <tr className="bg-yellow-50 border-b border-yellow-100">
      <td colSpan={9} className="px-8 py-5">
        <EditForm senior={senior} onClose={onClose} />
      </td>
    </tr>
  )
}
...
export function AdminView({
  ...
  return (
    <>
      {/* 통계 배너 */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 bg-white rounded-2xl border border-gray-200 p-4">
        <div className="text-center">
          <p className="text-xs text-gray-500 mb-1">전체 시니어</p>
          <p className="text-2xl font-bold">{totalSeniors}<span className="text-sm font-normal text-gray-400 ml-1">명</span></p>
        </div>
        <div className="text-center">
          <p className="text-xs text-gray-500 mb-1">배정 완료</p>
          <p className="text-2xl font-bold text-green-600">{assignedCount}<span className="text-sm font-normal text-gray-400 ml-1">명</span></p>
        </div>
        <div className="text-center">
          <p className="text-xs text-gray-500 mb-1">취업 완료</p>
          <p className="text-2xl font-bold text-emerald-600">{doneCount}<span className="text-sm font-normal text-gray-400 ml-1">명</span></p>
        </div>
        <div className="text-center">
          <p className="text-xs text-gray-500 mb-1">배정률</p>
          <p className="text-2xl font-bold text-blue-600">{assignRate}<span className="text-sm font-normal text-gray-400 ml-1">%</span></p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-6">
        {TABS.map((tab) => (
          <button
            ...
            className={`rounded-2xl border p-3 sm:p-6 text-center transition-colors cursor-pointer ${
              activeTab === tab.id
          ...
          >
            ...
            <p className="text-xs sm:text-lg font-semibold mb-1">{tab.label}</p>
            <p className="text-2xl sm:text-4xl font-bold">{counts[tab.id]}</p>
            <p className="text-xs sm:text-base opacity-60 mt-1">명</p>
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-4 rounded-xl border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900"
          />
        </div>
        <div className="flex gap-2">
          <select value={filterReg} onChange={(e) => setFilterReg(e.target.value)}
            className="flex-1 h-10 rounded-xl border border-gray-300 px-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-gray-900">
          ...
          {hasFilter && (
            <button onClick={() => { setSearch(''); setFilterReg(''); setFilterJob('') }}
              className="h-10 px-3 rounded-xl border border-gray-300 text-sm text-gray-500 hover:bg-gray-100 transition-colors whitespace-nowrap">
              초기화
            </button>
          )}
        </div>
        ...
        <>
                    ...
                    <React.Fragment key={senior.id}>
                      <tr className={`border-b border-gray-100 ${senior.memo ? 'bg-yellow-50/30' : ''}`}>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="flex gap-2">
                            ...
                            {activeTab === 'unmatched' && (
                              <button onClick={() => handleDelete(senior.id)} disabled={isDeleting || isPending}
                      ...
                      {activeTab === 'pending' && isExpanded && panelType === 'assign' && (
                        <tr className="bg-blue-50 border-b border-blue-100">
                          <td colSpan={9} className="px-8 py-5">
                            <p className="text-base font-semibold text-blue-800 mb-3">{senior.name} 님 — 배정할 공고를 선택하세요</p>
                            <div className="flex flex-col gap-2">
                              {matches.map((match) => (
                                <div key={match.id} className="flex items-center justify-between bg-white rounded-xl border border-blue-200 px-5 py-3">
                                  <div className="flex items-center gap-4">
                                    <span className={`text-base font-bold px-3 py-1 rounded-lg ${scoreBadgeClass(match.score)}`}>{match.score}점</span>
                                    <div>
                                      <p className="text-base font-semibold">{match.jobs.title}</p>
                                      <p className="text-sm text-gray-500">{match.jobs.region} · {match.jobs.job_type} · 최소 {match.jobs.required_career}년</p>
                                    </div>
                                  </div>
                                  <button onClick={() => handleAssign(match.id)} disabled={isPending}
                      ...
                      {isExpanded && panelType === 'edit' && <EditPanel senior={senior} onClose={closePanel} />}
                    </React.Fragment>
                  )
                })}
              </tbody>
          ...
          <div className="sm:hidden flex flex-col gap-3">
            {rows.map(({ senior, matches }) => {
              ...
              return (
                <React.Fragment key={senior.id}>
                  <div className={`bg-white rounded-2xl border border-gray-200 shadow-sm p-4 ${senior.memo ? 'border-yellow-300' : ''}`}>
                    {/* 헤더 */}
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="text-lg font-bold">
                          {senior.name}
                          {senior.memo && <span className="ml-1 text-sm text-yellow-600" title={senior.memo}>📝</span>}
                        </p>
                        {senior.phone && <p className="text-sm text-gray-500 mt-0.5">{senior.phone}</p>}
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        {topScore > 0
                          ? <span className={`text-sm font-bold px-2 py-0.5 rounded-lg ${scoreBadgeClass(topScore)}`}>{topScore}점</span>
                          : null}
                        {activeTab === 'assigned' ? (
                          ...
                          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${activeTab === 'unmatched' ? 'bg-gray-100 text-gray-500' : matchStatusBadge(topStatus)}`}>
                            {activeTab === 'unmatched' ? '미매칭' : matchStatusLabel(topStatus)}
                          </span>
                        )}
                      </div>
                    ...
                    {/* 정보 */}
                    <div className="flex gap-2 flex-wrap text-sm text-gray-600 mb-3">
                      <span className="bg-gray-100 px-2 py-0.5 rounded">{senior.region}</span>
                      <span className="bg-gray-100 px-2 py-0.5 rounded">{senior.desired_job}</span>
                      <span className="bg-gray-100 px-2 py-0.5 rounded">경력 {senior.career_years}년</span>
                      {senior.desired_salary ? <span className="bg-gray-100 px-2 py-0.5 rounded">{senior.desired_salary}만원</span> : null}
                    </div>

                    {activeTab === 'assigned' && assignedMatch && (
                      <p className="text-sm font-medium text-gray-700 mb-3">📌 {assignedMatch.jobs.title}</p>
                    )}

                    {/* 액션 버튼 */}
                    <div className="flex flex-wrap gap-2">
                      <Link href={`/recommendations?senior_id=${senior.id}`}
                        className="h-9 px-3 rounded-lg border border-gray-300 text-sm font-semibold text-gray-700 flex items-center hover:border-gray-900 transition-colors">
                        상세
                      </Link>
                      <button onClick={() => openPanel(senior.id, 'edit')}
                        className={`h-9 px-3 rounded-lg text-sm font-semibold transition-colors border ${isExpanded && panelType === 'edit' ? 'bg-yellow-100 border-yellow-400 text-yellow-800' : 'border-gray-300 text-gray-700'}`}>
                        수정
                      </button>
                      {activeTab === 'pending' && (
                        <button onClick={() => openPanel(senior.id, 'assign')}
                          className={`h-9 px-3 rounded-lg text-sm font-semibold transition-colors ${isExpanded && panelType === 'assign' ? 'bg-gray-700 text-white' : 'bg-gray-900 text-white'}`}>
                          {isExpanded && panelType === 'assign' ? '닫기 ▲' : '공고 선택 ▼'}
                        </button>
                      )}
                      {activeTab === 'assigned' && assignedMatch?.status === 'assigned' && (
                        <>
                          <button onClick={() => handleComplete(assignedMatch.id)} disabled={isPending}
                            className="h-9 px-3 rounded-lg bg-emerald-600 text-white text-sm font-semibold disabled:opacity-50">
                            {actionId === assignedMatch.id ? '처리 중…' : '취업 완료'}
                          </button>
                          <button onClick={() => handleUnassign(assignedMatch.id)} disabled={isPending}
                            className="h-9 px-3 rounded-lg border border-orange-400 text-orange-600 text-sm font-semibold disabled:opacity-50">
                            {actionId === assignedMatch.id ? '처리 중…' : '배정 취소'}
                          </button>
                        </>
                      )}
                  ...
                  {activeTab === 'pending' && isExpanded && panelType === 'assign' && (
                    <div className="bg-blue-50 rounded-2xl border border-blue-200 p-4">
                      <p className="text-sm font-semibold text-blue-800 mb-3">{senior.name} 님 — 배정할 공고를 선택하세요</p>
                      <div className="flex flex-col gap-2">
                        {matches.map((match) => (
                          <div key={match.id} className="bg-white rounded-xl border border-blue-200 p-3">
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-2 min-w-0">
                                <span className={`text-sm font-bold px-2 py-0.5 rounded-lg shrink-0 ${scoreBadgeClass(match.score)}`}>{match.score}점</span>
                                <div className="min-w-0">
                                  <p className="text-sm font-semibold truncate">{match.jobs.title}</p>
                                  <p className="text-xs text-gray-500">{match.jobs.region} · {match.jobs.job_type}</p>
                                </div>
                              </div>
                              <button onClick={() => handleAssign(match.id)} disabled={isPending}
                                className="h-9 px-3 rounded-lg bg-gray-900 text-white text-sm font-semibold shrink-0 disabled:opacity-50">
                                {actionId === match.id ? '…' : '배정'}
                              </button>
                            </div>
                          </div>
                        ))}
                  ...
                  {isExpanded && panelType === 'edit' && (
                    <div className="bg-yellow-50 rounded-2xl border border-yellow-200 p-4">
                      <EditForm senior={senior} onClose={closePanel} />
                    </div>
                  )}
                </React.Fragment>
              )
